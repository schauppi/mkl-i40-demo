...
The model is loaded once at process startup (module level), so every incoming
...
# The default resolves to the same directory as this file, matching where
# download_files.sh places the model and where the Dockerfile copies it.
MODEL_PATH = os.getenv("MODEL_PATH", str(Path(__file__).resolve().parent / "rf_vent4.pkl"))
